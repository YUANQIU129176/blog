...
exports.updateById = function(data, cb) {
        connection.query(`update users set name='${data.name}',pass='${data.pass}',avator='${data.avator}' where id=${data.id}`, function(err, results) {
            if (err) {
                throw err;
            }
            cb(results);
...
// 评论页面评论的总条数
exports.commentNum = function(id, cb) {
    connection.query(`update posts set comments=comments+1 where id = ${id}`, function(err, results) {
        if (err) {
            throw err;
        }
        cb(results);
...
// 浏览量
exports.commentsReadNUm = function(id, cb) {
    connection.query(`update posts set pv=pv+1 where id = ${id}`, function(err, results) {
        if (err) {
            throw err;
        }
        cb(results);
